...
the pdf which is multivariate gaussian.
"""
function bayesian_integral_exponential(X::Array{Float64,2}, f::Array{Float64,1}, cov_func_parameters::gaussian_kernel_hyperparameters, noise::Float64 = 0.0 )
    ndim = length(cov_func_parameters.w_i)
    K = K_matrix(X, BayesianIntegral.gaussian_kernel, cov_func_parameters, noise)
    invK = inv(K)
    z_val = cov_func_parameters.w_0 *  prod.(sqrt.(2 .* pi .* cov_func_parameters.w_i))
    z = repeat([z_val], ndim)
    expectation = transpose(z) * invK * f
    # Note variance is not yet supported.
