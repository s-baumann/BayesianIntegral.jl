...

{
    ...
    "title": "2.0 Training Hyperparameters",
    "category": "section",
    "text": "This package currently supports two ways of training hyperparameters. The first is"
},

{
    ...
    "category": "section",
    "text": "This is done by the calibratebyMLwithSGD function. The procedure is:Extract a sample of the requested size from the dataset. Sampling is done without replacement (or else the K matrix is singular and not invertible)\nFind the likelihood of the dataset given the input hyperparameters. Also find the marginal likelihood (with respect to all hyperparameters) and use the Newtonian method to suggest another set of hyperparameters. The step to the new parameterset (as chosen by the Newtonian method) can be adjusted by the step_multiple parameter.This process continues for a user-specifiable number of iterates.Note that using stochastic gradient descent is important here as the major time here is in inverting an N x N matrix which has a complexity of about O(N^23). Thus if only 10\\% of observations are used in each iterate this makes the calibration more than 100 times faster than using all observations.This function can be used in the following way:old_cov_func_parameters = gaussian_kernel_hyperparameters(1.0, repeat([10.0] , outer = dims))\nsteps = 100                           # How many optimisation steps\nbatch_size = 5                        # Number of observations per sample\nstep_multiple = 1.0                   # How far to step\nnoise = 0.001                         # Noise parameter\nseed = 2                              # Random number seed (for gathering random samples)\nnew_cov_func_parameters = calibrate_by_ML_with_SGD(X, y, cov_func_parameters, steps, batch_size, step_multiple, noise, seed)"
},
